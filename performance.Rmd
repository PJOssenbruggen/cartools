...
Since most travelers take traffic congestion for granted and give it little thought, it may seem implausibe that describing the unititation of a simple __traffic breakdown__ event requires two figures and a table of critical times, speeds and locations.  Let's take in turn. First, we will limit the discussion to three vehicles entering a bottleneck, two traffic lanes merge into one lane.  

The chance of breakdown is high, $\pi(k = 50)$ = 0.8 because the traffic density is heavy, $k$ = 50 vpm. At time $t_0$ = 0 seconds, the distance between vehicles 0 and 1, which is directly behind, is $h_0$ = 105 feet, a space headway estimated as $h_0 = 1/k$. $h_0$ is almost twice the safe headway $h_{safe}$ for an average speed of $u$ = 41 mph = 60 fps at this density. The positions of the three vehicles at $t_0$ = 0 seconds is shown under the table column heading $x_0$. The table shows there is no conflict between vehicles 0 and 1 and indicates that vehicle 0 can move freely downstream through the merge zone, $-500 < x \le 0$. Since the driver has no conflicts, we call a _zipper merge_.
