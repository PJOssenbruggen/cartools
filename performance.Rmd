...
Since most travelers take traffic congestion for granted and give it little thought, it may seem implausibe that describing the unitiating of a simple __traffic breakdown__ event involving only three vehicles requires two figures and a table of critical times, speeds and locations.  Let's take a look. First, we will limit the discussion to vehicles entering a bottleneck, two traffic lanes merging into one lane.  

 The roadway system is under stress because (1) vehicles are forced to merge, (2) traffic volatility is high, (3) drivers have limited room to manuever because the assumed traffic density of $k$ = 50 vpm is high,  (4) the traffic volatity, $\sigma$ = 11 mph = 16 fps, is high and (5) local clustering of vehicles as they move downstream plays a role. In the following simulation, two  of the three vehicles are traveling side-by-side before they enter the merge zone of the bottleneck, They become clustered. To avoid a crash, one driver must yield and decelerate. Given this combination of factors, traffic breakdown is expected to be triggered. The chance of a breakdown, $\pi(k = 50)$ = 0.8. As will be shown, a queue forms. It does not form abruptly but grows over time.    
 

Let us look at the simulation details. At time $t_0$ = 0 seconds, the distance between vehicles 0 and 1, which is directly behind, is $h_0$ = 105 feet, a space headway estimated as $h_0 = 1/k$. $h_0$ is almost twice the safe headway $h_{safe}$ for an average speed of $u$ = 41 mph = 60 fps at this density. The positions of the three vehicles at $t_0$ = 0 seconds is shown under the table column heading $x_0$. The table shows there is no conflict between vehicles 0 and 1 and indicates that vehicle 0 can move freely downstream through the merge zone, $-500 < x \le 0$. Since the driver has no conflicts, we call a _zipper merge_.
...
df8  <- data.frame(c("_Zipper_","Leader","","Follower"))
...
```
 ...
 Since I am here I am not there. Certainly. 
