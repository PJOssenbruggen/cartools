...
brktrials2(5, 30, 41, 11, xstart, -500, 14, lane, 0.5, 2) -> output
```
...
```{r bottleneck_merge2,  message = FALSE, fig.height = 24, fig.width = 8, echo = FALSE}
par(mfrow = c(1,2))
library(Basic)
library(rlist)
set.seed(127)
...
brktrials2(14, 45, 41, 11, xstart, -500, 14, lane, 0.1, 2) -> output
```

_As you can see, the code must be robust to account for volatility in the breakdown process. More debugging is necessary to remove quirky behavior. For instance, some vehicles are incorrectly backing up! My guess is that more the Brownian motion model needs work._ 

__Regardless, the brktrials2 function shows a queue to form in a short period of time and the drivers are affected well upstream of the merge zone.__ Realistic? I think so.


__Delay:__ As mentioned, the next step is to conduct an experiment consisting of 100 simulations and report our findings:
