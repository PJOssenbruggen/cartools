...
vehorder <- lst[[3]]
xfunnel  <- -500
plotmerge3(tuxv.fix, vehorder, xfunnel)
```
...
Given $\pi(k)$ = 0.8 at $k$ = 50 vpm and $\bar{u} \pm \sigma$ is so wide, the results from the two independent analyses taken together suggest a congestion event $X = 1$ will occur around 80$\%$ of the time. Another way of looking at the prediction is to state the bottleneck will have vehicles operating at speeds 41 mph or more 20$\%$ of the time. 
