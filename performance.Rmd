...
We have developed two modeling types to simulate traffic behavior.  Each type realisticly simulates conditions found in the field  using $t-x$ trajectories as the principle means of comparison. The two approaches are fundamentally different. One is a deterministic model and the other is a stochastic model. The forecasts from the two models compliment one another. Both satisfy the principal aim of developing the __cartools__ package: to explain highway performance as simply as possible.  
