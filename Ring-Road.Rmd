...
After time $\tau$ = 15 minutes, the Brownian Bridge model forces a driver in a mathematical sense to return to his or her initial speed $u(k)$, the speed at time $t$ = 0. Between the start and finish times of $t$ = 0 and 15 minutes, the drivers are free to decelerate and accelerate as they wish. The transitions over time are more-or-less smooth as expected. The effect of volatility is in clear evidence. According to the model, a driver can maintain a speed outside the 95% C.I. (confidence interval) for an extended time period as shown by the "Heavily Congested" simulation. It is emphasized that the Brownian Bridge model is for a single vehicle. Our next step is to consider driver interaction, which have not done as yet.
